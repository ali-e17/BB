...
class DictionaryActivity : AppCompatActivity() {
    ...
    // 🌟 لیست کلمات نامناسبی که پیدا کردی رو دقیقاً اینجا داخل دبل‌کوتیشن بنویس و با کاما جدا کن
    // حتماً کلمات رو با حروف کوچک (lowercase) اینجا وارد کن
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 🎲 منطق دکمه کلمه تصادفی (لیست سیاه فقط اینجا اعمال می‌شود)
        btnRandomWord.setOnClickListener {
            var foundSafeWord = false
            var attempts = 0
            var randomEntry: DictionaryEntry? = null

            // برنامه تا ۱۰ بار تلاش می‌کنه تا یک کلمه مطمئن که توی لیست سیاه نیست پیدا کنه
            while (!foundSafeWord && attempts < 10) {
                ...
                // چک می‌کنه که کلمه توی لیست سیاه نباشه
            ...
            if (randomEntry != null && foundSafeWord) {
                etSearch.text = null // پاک کردن کادر سرچ برای نمایش تمیز کلمه رندوم
        ...
        // 🔍 منطق جستجوی دستی کاربر (بدون هیچ فیلتر و محدودیت لیست سیاه)
        etSearch.addTextChangedListener(object : TextWatcher {
            override fun afterTextChanged(s: Editable?) {
                val query = s.toString().trim()
                if (query.isNotEmpty()) {
                    // مستقیم در دیتابیس سرچ می‌کنه؛ حتی اگر کلمه تو لیست سیاه باشه اینجا نشون داده میشه
