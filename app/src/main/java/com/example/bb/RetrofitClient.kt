...
interface ApiService {
    ...
    fun createAnnouncement(
        ...
        @Part("targetClassIds") targetClassIds: RequestBody,
    ...
    @GET("get_unread_announcement_count.php")
    fun getUnreadAnnouncementCount(): Call<UnreadAnnouncementCountResponse>

    @POST("create_attendance_announcements.php")
    fun createAttendanceAnnouncements(
        @Body request: AttendanceAnnouncementsRequest
    ): Call<AttendanceAnnouncementsResponse>

    @GET("get_attendance_overview.php")
    fun getAttendanceOverview(
        @Query("class_id") classId: String
    ): Call<AttendanceOverviewResponse>

    @GET("get_attendance_session.php")
    fun getAttendanceSession(
        @Query("class_id") classId: String,
        @Query("session_number") sessionNumber: Int
    ): Call<AttendanceSessionResponse>

    @POST("finalize_attendance.php")
    fun finalizeAttendance(
        @Body request: FinalizeAttendanceRequest
    ): Call<AttendanceSaveResponse>
...
data class AttendanceAnnouncementItemRequest(
    val studentId: String,
    val status: String,
    val delayMinutes: Int
)

data class AttendanceAnnouncementsRequest(
    val classId: String,
    val date: String,
    val items: List<AttendanceAnnouncementItemRequest>
)

data class AttendanceAnnouncementsResponse(
    val status: String = "",
    val message: String = "",
...
object RetrofitClient {
    ...
    fun attendanceExportUrl(classId: String): String {
        val encodedClassId = java.net.URLEncoder.encode(classId, Charsets.UTF_8.name())
        return "${BASE_URL}export_attendance_excel.php?class_id=$encodedClassId"
    }
    ...
    val instance: ApiService by lazy {
        check(::appContext.isInitialized) {
            "RetrofitClient.init(context) must be called from Application.onCreate()"
        }
