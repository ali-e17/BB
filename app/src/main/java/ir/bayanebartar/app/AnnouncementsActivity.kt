...
class AnnouncementsActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        NetworkMonitor.observe(this) { online ->
            if (online && !isFinishing) {
                lifecycleScope.launch {
                    loadAnnouncements(showLoader = false)
                }
            }
        }
    ...
    override fun onResume() {
        super.onResume()
        // اول کش محلی را نمایش بده؛ شبکه فقط برای Sync است.
        renderLocalCache()
        loadAnnouncements(showLoader = false)
    }

    private fun loadAnnouncements(showLoader: Boolean = true) {
    ...
    private fun renderLocalCache() {
        ...
        val local = when (mailbox) {
            ...
            Mailbox.INBOX -> allLocal
        }.map {
            if (mailbox == Mailbox.INBOX) it.copy(isRead = AppDatabase.isAnnouncementRead(it.id, role, identityKey))
            else it.copy(isRead = true)
        }
        if (local.isNotEmpty()) render(local)
    }
