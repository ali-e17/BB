...
class StudentManagementActivity : BaseActivity() {
    ...
    override fun onResume() {
        super.onResume()
        setupClassFilter()
        DataSyncManager.watch(this) { runOnUiThread { setupClassFilter() } }
    }
    ...
    private fun refreshStudents() {
        // Always keep last successful data visible while offline.
        val local = AppDatabase.getAllStudents()
        if (local.isNotEmpty()) applyStudentList(local)
        if (!DataSyncManager.hasInternet(this)) return
    ...
    private fun applyStudentList(allStudents: List<StudentModel>) {
        val activeClassIds = activeClasses.map { it.id.trim() }.toHashSet()
        val filtered = allStudents.filter { it.isActive }.filter { student ->
            val q=normalizePersian(search)
            val text=normalizePersian("${student.firstName} ${student.lastName} ${student.name} ${student.studentCode} ${student.phone}")
            val okSearch=q.isBlank() || text.contains(q)
            val id=student.classId?.trim().orEmpty()
            val has=id.isNotBlank() && id in activeClassIds
            val okClass=when(selectedClassId){ null->true; NO_CLASS->!has; HAS_CLASS->has; else->id==selectedClassId }
            okSearch && okClass
        }
        adapter.updateList(filtered)
    }
