...
private class SessionInterceptor(private val context:Context):Interceptor {
    ...
    override fun intercept(chain:Interceptor.Chain):okhttp3.Response {
        ...
        /*
         * IMPORTANT:
         * A 401 by itself is not enough reason to logout.
         *
         * Previously every HTTP 401 (including temporary server problems,
         * permission changes, malformed responses, or an endpoint-specific
         * authentication failure) destroyed the local session. This caused
         * random "session expired" messages while the user's token was still
         * valid.
         *
         * Only backend-confirmed session invalidation should force login again.
         */
        if(response.code==401 && !request.url.encodedPath.endsWith("login.php")) {
            ...
            val mustRelogin =
                apiError?.code?.uppercase() in setOf(
                    "SESSION_EXPIRED",
                    "UNAUTHENTICATED",
                    "RELOGIN_REQUIRED"
                )
